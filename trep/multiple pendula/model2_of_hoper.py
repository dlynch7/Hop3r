...
def main(links, dt, tf, generalized_method, hybrid_wrenched):
    ...
    (t, q, u, T) = simulate_system(mvi, t_sol, u_sol, q_ref[0], q_ref[1])
...
def visualize(system, t, q, u=None):
    visual.visualize_3d([visual.VisualItem3D(system, t, q, u)])
...
def make_pendulum(num_links, hybrid_wrenched=False):
    ...
    def add_level(frame, link=0):
        ...
        child = trep.Frame(frame, trep.RX, config_name, "link-%d" % link)
...
def generate_reference(mvi, dt, tf):
    ...
    def forcing(t):
        ...
        u = [(5*(Nu-i+1)*sin(3.0*t + pi/4.0*i)) for i in range(Nu)]
        return tuple(u)
