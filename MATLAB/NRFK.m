...
while(convMeasure > thresh)
    ...
    convMeasure = abs(G0 - G1)/abs(G0); % compute convergence measure with candidate soln.
...
switch chain
    case 1 % calculate FK along theta-chain
        [xF, yF, angF] = subchainFK(theta,1);
    case 2 % calculate FK along phi-chain
        [xF, yF, angF] = subchainFK(phi,2);
    case 3 % calculate FK along psi-chain
        [xF, yF, angF] = subchainFK(psi,3);
