...
int8_t geomFK(float *qa, float *qu, float *footPose, uint8_t solOption) {
  ...
  ****************************************************************************/
  ...
  printf("%f\t%f\t%f\n",footX,footY,footAngle);

  /****************************************************************************
  * Calculate the third angle in each open chain:
  ****************************************************************************/
  gammatheta = footAngle - qa[0] - betatheta;
  gammaphi = footAngle - qa[1] - betaphi;
  gammapsi = footAngle - qa[2] - betapsi;
  ...
  printf("%f\t%f\t%f\n",betatheta,betaphi,betapsi);
  printf("%f\t%f\t%f\n",gammatheta,gammaphi,gammapsi);
  return 1;
}
